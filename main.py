...
ALLL = OTHER + NUMBERS + DATES_F + YESNO_F + EXACT_F   
...
def build_merged_df(fe: Field_Item): 
    ...
    if fe.cbonds_field == 'Perpetual (yes/no)': 
...
def rename_df(mdf, fe): 
    ...
    #have to include the empty cols
...
def clean_dfs(mmdf, msdf, fe,  match_col = True):
    ...
    #fields with empty missing tables 
    emtpy_missing = ['Floating rate (yes/no)','Сonvertable (yes/no)']
    if fe.cbonds_field in emtpy_missing:
...
def run_coupon(mdf, fe): 
    #getting rid of empty CBond values
    mdf = mdf[mdf['Coupon frequency'].isnull() == False]
    conditions = [
    ...
    #getting rid of empty WFI values
    mmdf = mmdf[mmdf['InterestPaymentFrequency'].isnull() == False]
...
def build_df(fe):
    ...
    if fe.cbonds_field in EXACT_F: 
        ...
        if fe.wfi_field == 'DebtCurrency':
            #extra rules, tuples of (cbonds, wfi)
            extra_matches = [('UVR', 'COU'),('UDI','MXV'),('BYR', 'BYN')]
            for cb, wf in extra_matches:
                df1 = mmdf[mmdf.iloc[:,1] == cb]
                df2 = df1[df1['DebtCurrency (EDI WFI)']==wf]
                mmdf = mmdf.drop(df2.index)
        ...
        if fe.cbonds_field == 'Mortgage bonds (yes/no)': 
            ...
            mdf['Match?'] = np.select(conditions, choices, default='')
            mmdf = mdf[mdf['Match?'] == 'mismatch']
            msdf = pd.DataFrame(columns=['No Missing Values'])
            return clean_dfs(mmdf, msdf, fe)
        ...
        if fe.wfi_field == 'SecuredBy':
            conditions = [
                ((mdf[fe.cbonds_field] == 1) & (mdf['SecuredBy'] == 'Y')) |
                ((mdf[fe.cbonds_field] == 2) & (mdf['SecuredBy'] == 'N')) |
                ((mdf[fe.cbonds_field] == 3) & (mdf['SecuredBy'] == 'N')) |
                ((mdf[fe.cbonds_field] == 4) & (mdf['SecuredBy'] == 'N')) |
                ((mdf[fe.cbonds_field] == 5) & (mdf['SecuredBy'] == 'N')) |                
                ((mdf[fe.cbonds_field] == 6) & (mdf['SecuredBy'] == 'Y')) |
                ((mdf[fe.cbonds_field] == 9) & (mdf['SecuredBy'] == 'Y')) |               
                ((mdf[fe.cbonds_field] == 0)) |((mdf[fe.cbonds_field] == 7)) | ((mdf[fe.cbonds_field] == 8)),
                ((mdf[fe.cbonds_field].isna() == False) & (~mdf[fe.cbonds_field].isin([0,7,8])) & (mdf['SecuredBy'].isna() == True))]
        
        choices = ['match', 'missing']
        mdf['Match?'] = np.select(conditions, choices, default='mismatch')
        mmdf = mdf[mdf['Match?'] == 'mismatch']
        msdf = mdf[mdf['Match?'] == 'missing']
        ...
        if fe.wfi_field == 'ConventionMethod': #1
            ...
            msdf = mdf[mdf['Match?'] == 'missing']
            return clean_dfs(mismatchdf, msdf, fe)

        if fe.wfi_field == 'InterestAccrualConvention': #0
            from rules import DCCLvl1
            conditions = [  
                ((mdf[fe.cbonds_field].isnull() == False) & (mdf[fe.wfi_field].isnull() == True))]
            choices = ['missing']
            mdf['Match?'] = np.select(conditions, choices, default='n')
        ...
        '''
        wfi csv is being read as y/m/d; cbonds csv is being read as d.m.y
        '''
       ...
       #mdf[fe.wfi_field] = mdf[fe.wfi_field].str[:10]
        ...
        #switching y/m/d to d/m/y
        mdf[['Year','Month','Day']] = mdf[fe.wfi_field].str.split('/',expand = True)
        mdf[fe.wfi_field] = mdf[['Day','Month','Year']].agg(lambda x: '/'.join(map(str,x)),axis = 1)
        mdf = mdf.drop(['Day','Month','Year'],axis = 1)

        nempt = mdf[mdf[fe.cbonds_field] != 'nan'] #saving value and none to missing
        msdf = nempt[nempt[fe.wfi_field] == 'None/None/nan']

        #after removing wfi's None's
        nempt = nempt[nempt[fe.wfi_field] != 'None/None/nan']
        conditions = [
            (nempt[fe.cbonds_field] != nempt[fe.wfi_field])]
        choices = ['mismatch']
        nempt['Match?'] = np.select(conditions, choices, default=' ')
        mmdf = nempt[nempt['Match?'] == 'mismatch']
